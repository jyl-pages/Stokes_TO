...
static ImVector<ImGuiStorage::ImguiPair>::iterator LowerBound(ImVector<ImGuiStorage::ImguiPair>& data, ImGuiID key)
{
    ImVector<ImGuiStorage::ImguiPair>::iterator first = data.begin();
    ImVector<ImGuiStorage::ImguiPair>::iterator last = data.end();
    int count = (int)(last - first);
    while (count > 0)
    {
        int count2 = count / 2;
        ImVector<ImGuiStorage::ImguiPair>::iterator mid = first + count2;
        if (mid->key < key)
        {
            first = ++mid;
            count -= count2 + 1;
...
{
    ImVector<ImguiPair>::iterator it = LowerBound(const_cast<ImVector<ImGuiStorage::ImguiPair>&>(Data), key);
...
{
    ImVector<ImguiPair>::iterator it = LowerBound(Data, key);
    if (it == Data.end() || it->key != key)
        it = Data.insert(it, ImguiPair(key, default_val));
    return &it->val_i;
}
...
void ImGuiStorage::SetInt(ImGuiID key, int val)
{
    ...
    if (it == Data.end() || it->key != key)
    {
        Data.insert(it, ImguiPair(key, val));
        return;
    }
    it->val_i = val;
}
    ...
    if (ImGui::Begin("ImGui Metrics", p_open))
    {
        ...
        struct Funcs
        {
            ...
            static void NodeWindow(ImGuiWindow* window, const char* label)
            {
                ...
                ImGui::BulletText("Storage: %d bytes", window->StateStorage.Data.Size * (int)sizeof(ImGuiStorage::ImguiPair));
                ImGui::TreePop();
            }
        };
