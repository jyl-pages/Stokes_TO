...
real FluidTopoOpt::Compute_Objective(const real * var)
{
	MMA2Fluid(var, design);
	return (real)Obj();
}

void FluidTopoOpt::Compute_Gradient(const real * var, real * grad)
{
	MMA2Fluid(var, design);
	Update_Grad();
...
void FluidTopoOpt::Compute_Constraint(const real * var, real * constraint)
{
	constraint[0] = 0.0;
...
void FluidTopoOpt::Compute_Constraint_Grad(const real * var, real * constraint_grad)
{
	for (int i = 0; i < n_var; i++) {
...
void FluidTopoOpt::Write_Substep(const int frame)
{
	std::cout << "Optimization iteration " << frame << std::endl;
	if (Write_Output_Files_Callback)Write_Output_Files_Callback(frame);
}

void FluidTopoOpt::Init(int _grid_size, Scalar _frac, int _max_iter)
{
	...
	design.Resize(Vector2i(grid_size, grid_size));
	design_grad.Resize(Vector2i(grid_size, grid_size));

	buffer.Resize(Vector2i(grid_size, grid_size));

	Init_MMA();
}

void FluidTopoOpt::Init_MMA()
{
...
void FluidTopoOpt::Init_Boundary(const Field<Scalar, 2>& cell_vol, const Field<int, 2>& cell_fixed, const Field<Scalar, 2>& _cell_b, \
	const FaceField<Scalar, 2>& face_vol, const FaceField<int, 2>& face_fixed, const FaceField<Scalar, 2>& _face_b, const Field<Scalar,2>& _cell_penalty)
{
...
Scalar FluidTopoOpt::alpha(Scalar rho)
{
	//return rho;
...
Scalar FluidTopoOpt::D_alpha(Scalar rho)
{
	//return (Scalar)1;
...
void FluidTopoOpt::MMA2Fluid(const real *v, Field<Scalar, 2>& f)
{
...
void FluidTopoOpt::Fluid2MMA(const Field<Scalar, 2> f, real *v)
{
	iterate_cell_d(iter, grid, 2)
	{
		const Vector2i& cell = iter.Coord();
...
void FluidTopoOpt::Update_Fluid()
{
	Timer<real> timer;
	timer.Reset();
	...
	iterate_cell_d(iter, grid, 2)
	{
		...
		face_penalty(0, cell) += t;
		face_penalty(0, cell + Vector2i::Unit(0)) += t;
		face_penalty(1, cell) += t;
		face_penalty(1, cell + Vector2i::Unit(1)) += t;
	}
...
void FluidTopoOpt::Update_Grad()
{
	...
	iterate_cell_d(iter, grid, 2)
	{
		...
		Scalar sum = (Scalar)0;
		sum+=temp_grad(0, cell);
		sum+=temp_grad(0, cell + Vector2i::Unit(0));
		sum+=temp_grad(1, cell);
		sum+=temp_grad(1, cell + Vector2i::Unit(1));
		design_grad(cell) = sum * t;
	}
	timer.Elapse_And_Output("gradient");
}

Scalar FluidTopoOpt::Obj()
{
	Update_Fluid();
	return energy.compute_energy(vel, p);
}

void FluidTopoOpt::Numerical_Derivative()
{
	Field nd = design;
	Scalar dx = 1e-2;
