...
void Poisson3D::init(int _grid_size, Scalar _dx)
{
	grid_size = _grid_size;
	grid.Initialize(Vector3i(grid_size, grid_size, grid_size), _dx);
	l = round(std::log2(grid_size&-grid_size)) - 1;

	x.Resize(Vector3i(grid_size, grid_size, grid_size));
	b.Resize(Vector3i(grid_size, grid_size, grid_size));

	PoissonDescriptor3D descr = PoissonDescriptor3D();
	...
	mg_descr = new PoissonDescriptor3D[l];
	mg_descr[l - 1] = descr;
...
void Poisson3D::solve()
{
	...
	const PoissonDescriptor3D& descr = mg_descr[l - 1];
...
{
	PoissonDescriptor3D& descr = mg_descr[l - 1];
