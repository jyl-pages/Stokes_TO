...
void StokeFlowMapping3D::init(StokeFlowDescriptor3D *_descr)
{
	descr = _descr;
	Nx = descr->Nx;
	Ny = descr->Ny;
	...
	cudaMalloc(&d_temp, sizeof(Scalar)*(dof + grid.edge_size(0) + grid.edge_size(1) + grid.edge_size(2)));
}

int StokeFlowMapping3D::xDoF()
{
	return dof;
}

int StokeFlowMapping3D::yDoF()
{
	return dof;
}

void StokeFlowMapping3D::applyMapping(Scalar *Ap, Scalar *p)
{
	int cell_off = 0;
	...
	// - curl curl v
	{
		...
		//grid3DOperator::Cod1Mapping(grid, d_temp + edge_x_off, d_temp + edge_y_off, d_temp + edge_z_off, p + face_x_off, p + face_y_off, p + face_z_off);
		...
		//cwise_mapping_wrapper(d_temp + edge_y_off, neg, grid.edge_size(1));
	...
	// div v
	{
		...
		cwise_mapping_wrapper(Ap + cell_off, d_temp + cell_off, add, grid.cell_size());
	}
	...
	// penalty
	{
		...
		cwise_mapping_wrapper(Ap + cell_off, d_temp, add, dof);
	}
}

void StokeFlowMapping3DFixed::init(StokeFlowDescriptor3D *_descr)
{
	descr = _descr;
	Nx = descr->Nx;
	Ny = descr->Ny;
...
int StokeFlowMapping3DFixed::xDoF()
{
	return dof;
}

int StokeFlowMapping3DFixed::yDoF()
{
	return dof;
}

void StokeFlowMapping3DFixed::applyMapping(Scalar *Ap, Scalar *p)
{
	int cell_off = 0;
	int face_x_off = cell_off + grid.cell_size();
	int face_y_off = face_x_off + grid.face_size(0);
